...
const OrderPage: React.FC = () => {
  ...
  // --- TUTAJ ZACZYNAJĄ SIĘ ZMIANY (NOWA LOGIKA) ---
  ...
  const prepareAddressData = (data: AddressData) => {
    ...
    const lastName = nameParts.slice(1).join(" ") || "-"; // Fallback, gdyby ktoś podał tylko imię
  ...
  const handleOrderSubmit = async () => {
    ...
    // Walidacja czy mamy pricingId (bez tego nie wyślemy requestu)
    ...
    try {
      ...
      // Wywołujemy zaktualizowaną funkcję sendPackage
      const response = await sendPackage({
        pricingId: state.offer.pricingId, // <--- PRZEKAZUJEMY ID
      ...
      if (error.response) {
        // Wyświetlamy treść błędu HTML w konsoli, żeby podejrzeć szczegóły w razie czego
        console.error("Szczegóły błędu (body):", error.response.data);
        
        // Prostym językiem dla użytkownika
