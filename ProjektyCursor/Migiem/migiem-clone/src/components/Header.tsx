...
import { Plane, User, LogOut, Menu, X } from "lucide-react"; // Dodałem ikony

export default function Header() {
  const [isLoggedIn, setIsLoggedIn] = useState(false);
  const [isMenuOpen, setIsMenuOpen] = useState(false); // Do menu mobilnego
  const navigate = useNavigate();

  // Sprawdzamy status logowania przy załadowaniu
  useEffect(() => {
    ...
    // Nasłuchuj zmian w localStorage (opcjonalne, dla płynności)
    ...
    <header className="sticky top-0 z-50 border-b bg-white shadow-sm font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Navigation Links - zamieniłem <a> na <Link> */}
          ...
          {/* Action Buttons - TU JEST LOGIKA */}
          <div className="hidden md:flex items-center gap-3">
            {isLoggedIn ? (
              // WERSJA ZALOGOWANA (Twój styl)
              <>
              ...
              // WERSJA NIEZALOGOWANA (Oryginalna)
              <>
                <Link to="/login">
          ...
          {/* Mobile Menu Button (Dodałem, żeby działało na telefonach) */}
          <button 
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-white border-t border-gray-100 p-4 space-y-4 shadow-lg animate-in slide-in-from-top-2">
          <nav className="flex flex-col gap-4">
            <Link to="/" className="text-gray-600 font-medium">Oferta</Link>
            <Link to="/" className="text-gray-600 font-medium">Dla Firm</Link>
            ...
            {isLoggedIn ? (
                ...
                <>
                    <Link to="/login" className="text-gray-700 font-medium">Zaloguj</Link>
                    <Link to="/register" className="text-blue-600 font-medium">Załóż konto</Link>
                </>
            )}
          </nav>
        </div>
